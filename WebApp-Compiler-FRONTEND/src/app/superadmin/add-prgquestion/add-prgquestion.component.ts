...
@Component({
  selector: 'app-add-prgquestion',
  templateUrl: './add-prgquestion.component.html',
  styleUrls: ['./add-prgquestion.component.css']
})
export class AddPRGQuestionComponent implements OnInit {
...
Extractoutput()
{
  ...
  this.httpservice.postdata("http://localhost:8086/admin/execute",this.codeData).subscribe((item: any)=>
  {
    ...
    {
      this.showLoadingPopup=false
      this.prgmodel.basic=item.extractedFunction
      Swal.fire({
        background: "#2ecc71",
        color:"#fff",
        toast: true,
