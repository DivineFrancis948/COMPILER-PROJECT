...
//	private final ExecutorService threadPool = Executors.newFixedThreadPool(1); // Adjust the number of threads as
	...
	private static Logger logger = LogManager.getLogger(CCompilerServiceImpl.class);
	Integer testCaseId = 1;

	public Response checkUserCProgram(EmbeddedidDTO id, UUID randomUUID) throws Exception {
		try {
...
//            deletionService.deleteFilesWithPrefix(".", "temp");
			...
			try {
				...
				if (studentPrg.isPresent()) {
					...
					String cSourceCode = prg.getSolution();

					Response userResults = new Response();
					
					if(cSourceCode.isBlank()  || cSourceCode.isEmpty())
					{
...
//				userResults = executeUsercProgramUsingThreadPool(cSourceCode, id.getQuestionid(), "user",
//						randomUUID).get();
					userResults = executeUsercProgram(cSourceCode, id.getQuestionid(), "user",
							randomUUID);
...
//				userResults = null;
					...
					if (userResults == null) {
						userResults = checkUsercProgramWithRetry(cSourceCode, id.getQuestionid(),
								"user", randomUUID);
...
//		return null;
	...
	private Response checkUsercProgramWithRetry(String cSourceCode, String id, String role, UUID randomUUID) {
		try {
			int maxRetries = 2;
			int retryCount = 0;
			...
			while (retryCount < maxRetries) {
				try {
					userResults = executeUsercProgram(cSourceCode, id, role, randomUUID);
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private Response executeUsercProgram(String cSourceCode, String id, String role, UUID randomUUID)
...
//			resetSharedState();
			...
			if (vulnerablityCheckFlag) {
				boolean isClean = scanExecutableWithDivineAV(cFile + ".exe");
				if (!isClean) {
...
//	                }
	...
	public Response checkCustomUserCProgram(CustomTestDTO dto,UUID randomUUID) throws TestCaseFailException {
		
			try {
				FileDeletionService deletionService = new FileDeletionService();
				deletionService.deleteFilesWithPrefix(".", "temp");
				String cSourceCode = dto.getProgram();
				String testCase = dto.getInput();
				String role = "user";
				if(cSourceCode == null || testCase == null || role == null)
				{
					logger.error("some of the required fields in custominput is missing");
					return null; //need to address later
				}
				
				Response response = executeCustomUsercProgram(cSourceCode,testCase,role,randomUUID);
	...
	private Response executeCustomUsercProgram(String cSourceCode, String testCases, String role, UUID randomUUID)
			throws TestCaseFailException {
		try {
			File cFile;
			summary.clear();
