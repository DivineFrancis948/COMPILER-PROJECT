...
//	private final ExecutorService threadPool = Executors.newFixedThreadPool(1); // Adjust the number of threads as
	...
	private static Logger logger = LogManager.getLogger(JavaCompilerServiceImpl.class);
	Integer testCaseId = 1;
	
	public Response checkUserJavaProgram(EmbeddedidDTO id, UUID randomUUID) throws Exception {
		try {
...
//            deletionService.deleteFilesWithPrefix(".", "temp");
			...
			try {
				...
				if (studentPrg.isPresent()) {
					...
					String javaSourceCode = prg.getSolution();

					Response userResults = new Response();
					
					if(javaSourceCode.isBlank()  || javaSourceCode.isEmpty())
					{
					...
					userResults = executeUserJavaProgram(javaSourceCode, id.getQuestionid(), "user",
							randomUUID);

//				userResults = null;

					if (userResults == null) {
						userResults = checkUserJavaProgramWithRetry(javaSourceCode, id.getQuestionid(),
								"user", randomUUID);
...
//		return null;
	...
	private Response checkUserJavaProgramWithRetry(String javaSourceCode, String id, String role, UUID randomUUID) {
		try {
			int maxRetries = 2;
			int retryCount = 0;
			...
			while (retryCount < maxRetries) {
				try {
					userResults = executeUserJavaProgram(javaSourceCode, id, role, randomUUID);
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void deleteExistingExecutable(File javaFile) {
		...
		try {
			File existingExecutable = new File(javaFile.getAbsolutePath() + ".class");
			File existingFile = new File(javaFile.getAbsolutePath() );
	...
	private Response executeUserJavaProgram(String javaSourceCode, String id, String role, UUID randomUUID)
		...
		try {
			...
			if (vulnerablityCheckFlag) {
				boolean isClean = scanExecutableWithDivineAV(javaFile + ".class");
				if (!isClean) {
			...
			for (String testCaseSet : testCaseSets) {
				...
				try {
					result = executeCompiledBinary(javaFile.getName(), testCaseSet, role, id);
				} catch (TestCaseFailException e) {
...
//				 System.out.println(line);
	...
	private boolean executeTestCase(String testCaseLine, String javaClassName, String role, String id, boolean[] isTle)
...
//				isTle[0] = true;
	...
	private CompletableFuture<Boolean> executePgm(String testCaseLine, String javaClassName, String role, String id,
			boolean[] isTle) throws TestCaseFailException {
		try {
			boolean res = executeTestCase(testCaseLine, javaClassName, role, id, isTle);
	...
	private ExecutionResult executeCompiledBinary(String javaClassName, String testCases, String role, String id)
		...
		try {
			...
			for (String testCaseLine : testCaseLines) {
				...
				threads.add(executePgm(testCaseLine, javaClassName, role, id, isTle));
...
//	                }
	...
	public Response checkCustomUserJavaProgram(CustomTestDTO dto,UUID randomUUID) throws TestCaseFailException {
		
			try {
				FileDeletionService deletionService = new FileDeletionService();
				deletionService.deleteFilesWithPrefix(".", "temp");
				String javaSourceCode = dto.getProgram();
				String testCase = dto.getInput();
				String role = "user";
				if(javaSourceCode == null || testCase == null || role == null)
				{
					logger.error("some of the required fields in custominput is missing");
					return null; //need to address later
				}
				
				Response response = executeCustomUserJavaProgram(javaSourceCode,testCase,role,randomUUID);
	...
	private Response executeCustomUserJavaProgram(String javaSourceCode, String testCases, String role, UUID randomUUID)
			throws TestCaseFailException {
		try {
			File javaFile;
			summary.clear();
			...
			for (String testCaseSet : testCaseSets) {
				...
				try {
					result = executeCustomCompiledBinary(javaFile.getName(), testCaseSet, role); //nullpoiterex
				} catch (TestCaseFailException e) {
					logger.info("Time Limit Exceeded.. "+e.getMessage());
					throw e;
				}
	...
	private CustomExecutionResult executeCustomCompiledBinary(String binaryPath, String testCases, String role)
		...
		try {
			...
			CompletableFuture.allOf(toCfArray(threads)); //nullpointerex
	...
	private boolean executeCustomTestCase(String testCaseLine, String binaryPath, String role, boolean[] isTle)
		...
		try {
			// Create a new ProcessBuilder for each thread
			ProcessBuilder executeProcessBuilder = new ProcessBuilder("java",binaryPath);
			executeProcessBuilder.redirectErrorStream(true);
